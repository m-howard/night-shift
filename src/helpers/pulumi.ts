...
export function getVerifiedOutput(
    stackReference: pulumi.StackReference,
    outputName: string,
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
): pulumi.Output<any> {
    const output = stackReference.getOutput(outputName);
    return output.apply((outputValue) => {
        if (outputValue === undefined || outputValue === null) {
            throw new Error(`Output ${outputName} does not exist or has no value.`);
        }
        return outputValue;
    });
}
...
export function isTaggable(t: string): boolean {
    return TAGGABLE_RESOURCE_TYPES.indexOf(t) !== -1;
}
...
export function registerAutoTags(autoTags: Record<string, string>): void {
    try {
        ...
        // In unit-test environments the Pulumi root stack may not be
        // initialized yet; guard against that by making this a no-op.
        // Keep logging minimal to avoid huge error inspections in test
        // runners.

        console.warn('registerAutoTags skipped: runtime not initialized');
    }
}
